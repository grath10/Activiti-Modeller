...
angular.module('activitiModeler')
    ...
    if (!$rootScope.editorInitialized) {
        ...
        $rootScope.editorFactory.promise.then(function() {
            ...
            // to eliminate the tour.js with authentication injected
        ...
        jQuery(window).resize(function () {
            ...
            var totalAvailable = jQuery(window).height() - offset.top - mainHeader.height() || 40 - 21;
            // notice the changes made by own
            if(propSectionHeight <= 50) {
    			...
    			if (!$scope.unsavedChangesModalInstance) {
    				...
    				var handleResponseFunction = function (discard) {
    					...
    					if (discard) {
                            ...
                            $location.url(next.substring(next.lastIndexOf('#') + 1));
    					} else {
                    ...
                    _internalCreateModal({
    					template: 'editor/editor-app/popups/unsaved-changes.html',
    					scope: $scope
    				},  $modal, $scope);
    			}
    		}
